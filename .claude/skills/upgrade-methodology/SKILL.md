...
| **Harness config** | `.claude/settings.json` | If absent, copy it. If present, **keyed-merge** the kit's hook entries into the project's existing hooks (add-only, keyed by `matcher`+`command`): extend the `SessionStart` matcher (e.g. add `compact`) and add any new `PreToolUse`/`PostToolUse` hook, never overwriting or removing the project's own. The `specway upgrade` CLI does this automatically (`scripts/merge-hooks.mjs`); merge by hand only if upgrading without the CLI. |
